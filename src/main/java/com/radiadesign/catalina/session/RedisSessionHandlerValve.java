...
public class RedisSessionHandlerValve extends ValveBase {
    ...
    public void invoke(Request request, Response response) throws IOException, ServletException {
        ...
        RedisSessionManager manager = (RedisSessionManager) context.getManager();
        if (context == null) {
        ...
        try {
            ...
            storeOrRemoveSession(session, manager);
            manager.afterRequest();
        }
    }

    private void storeOrRemoveSession(Session session, RedisSessionManager manager) {
        try {
            if (session != null) {
                if (session.isValid()) {
